...
public final class AutoPlanterGameTest implements ModInitializer {
    ...
    public void onInitialize() {
        createTests().forEach(TestFunctionRegister::registerTestFunction);
        TestFunctionRegister.addFunctionsToRegistry(AutoPlanterCommon.AUTO_PLANTER, TestFunctionRegister::vanillaTestFunctionRegister);
    }

    private static List<TestFunction> createTests() {
        Map<String, BiConsumer<GameTestHelper, PlanterBlock>> tests = Map.ofEntries(
            ...
            Map.entry("placeSugarcaneTest", AutoPlanterGameTest::placeSugarcaneTest),
    ...
    static void placeSugarcaneTest(GameTestHelper helper, PlanterBlock block) {
        var pos = new BlockPos(0, 1, 0);
        helper.setBlock(pos, Blocks.AIR);
        helper.setBlock(pos.above(), Blocks.AIR);
        var seed = Items.SUGAR_CANE;
        helper.setBlock(pos, block.defaultBlockState().setValue(PlanterBlock.TRIGGERED, true));
        useBlock(helper, pos, helper.makeMockPlayer(GameType.CREATIVE), new ItemStack(seed), Direction.UP);
        helper.assertBlockPresent(Blocks.SUGAR_CANE, pos.above());
        helper.succeed();
    }

    static void hopperInsertTest(GameTestHelper helper, PlanterBlock block) {
